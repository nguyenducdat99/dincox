...
function CartItem(props) {
  ...
  // get max quantity
  const quantityMax = findQuantity(
    sizeDetailsRec,
  ...
  // get discount
  let discount = 0;
  if (item.product.is_sale && saleDetails.length > 0)
    discount =
      ConvertState.findDiscountForProduct(item.product.id_product, saleDetails)
        .length > 0
  ...
  return (
    <tr className="cart-item">
      ...
      <td>
        <b>
          {item.product.price * quantity * factor}
          <u>đ</u>
        </b>
      </td>
    </tr>
