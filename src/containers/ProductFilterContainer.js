...
function ProductFilterContainer(props) {
  ...
  const title =
    index === -1 ? "Tất cả các sản phẩm" : categories[index].category_name;

  // filter product using size
  var itemsFilter =
    valFilterPrice === null || valFilterPrice * 1 === 0
      ? products
      : onFilterPrice(valFilterPrice, products);

  itemsFilter =
    valFilterSize.length < 1 || valFilterSize.indexOf("0") > -1
      ? itemsFilter
      : onFilterSize(itemsFilter, sizeDetails, valFilterSize);
...
const onFilterPrice = (type, array) => {
  switch (type) {
    case "1":
      ...
      return result1;
    case "2":
      const result2 = array.filter((element) => {
        return element.price >= 100000 && element.price < 500000;
      });
      return result2;
    case "3":
      const result3 = array.filter((element) => {
        return element.price >= 500000 && element.price < 1000000;
      });
      return result3;
    case "4":
      const result4 = array.filter((element) => {
...
const onCheckFilterSize = (id_product, arraySize, types) => {
  // find size detail
  const sizeDetail = arraySize.filter((element) => {
